...
@Description: This file is for the NLP capstone of GreedyAI.com. 
    This file implement the transformer module.
@All Right Reserve
'''
...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, query, key, value, padding_mask):
        '''
        Task3
        This function implement the multihead attention.
        inputs:
            query: [batch_size, seq_len, n_features]
            key: [batch_size, seq_len, n_features]
            value: [batch_size, seq_len, n_features]
            padding_mask: [batch_size, seq_len]
        outputs:
            results of the multihead attention. A tensor of shape [batch_size, seq_len, n_features]
        This function should be implemented as follows:
        1. determine whether the future mask is needed. Only when the query, key and value are the same tensor, should we apply the future mask.
        2. make linear projections of the query, key and value.
        3. split the query, key and value into multiple heads. (use self._split_heads)
        4. apply the multihead attention. (use self._attn, with apply_future_mask={a proper value})
